                        ...
                        return (
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-[10px] font-mono text-gray-600">
                            ...
                            <div className="space-y-1.5 bg-gray-50 p-3 rounded-xl border border-gray-100">
                              ...
                              <div className="truncate" title={adv.hardware?.detailedWebGL ? `Vend: ${adv.hardware.detailedWebGL.vendor}, Rndr: ${adv.hardware.detailedWebGL.renderer}, MaxTex: ${adv.hardware.detailedWebGL.maxTextureSize}, Exts: ${adv.hardware.detailedWebGL.extensionsCount}` : ''}><span className="text-gray-400">GPU:</span> {adv.hardware?.detailedWebGL?.renderer || adv.hardware?.gpu}</div>
                              ...
                              <div className="truncate" title={adv.hardware?.advancedSensors}><span className="text-gray-400">SENS:</span> {adv.hardware?.advancedSensors || "N/A"}</div>
                            </div>
                            <div className="space-y-1.5 bg-gray-50 p-3 rounded-xl border border-gray-100">
                              ...
                              <div className="truncate" title={adv.behavior?.fieldFocusTimes && Object.keys(adv.behavior.fieldFocusTimes).length > 0 ? Object.entries(adv.behavior.fieldFocusTimes).map(([k,v]) => `${k}:${Number(v)/1000}s`).join(', ') : 'N/A'}><span className="text-gray-400">FOC/BLR/PST/CP/CT:</span> {adv.behavior?.fieldFocusTimes && Object.keys(adv.behavior.fieldFocusTimes).length > 0 ? Object.keys(adv.behavior.fieldFocusTimes).length : 0} flds / {adv.behavior?.blurCount} / {adv.behavior?.pastes || 0} / {adv.behavior?.copies || 0} / {adv.behavior?.cuts || 0}</div>
                              {adv.behavior?.autofillUsed && <div><span className="text-gray-400 text-orange-500 font-bold">AUTOFILL RILEVATO</span></div>}
                            ...
                            <div className="space-y-1.5 bg-gray-50 p-3 rounded-xl border border-gray-100 overflow-hidden">
                              ...
                              <div className="truncate"><span className="text-gray-400">PR:</span> {adv.software?.platform} {adv.software?.historyLength ? `(Hist: ${adv.software.historyLength})` : ''}</div>
                              ...
                              <div><span className="text-gray-400">BOT/INC:</span> {adv.software?.botStatus || 'N/A'} / {adv.software?.incognito || 'N/A'}</div>
                              <div className="truncate" title={adv.software?.performanceMemory ? `L:${adv.software.performanceMemory.jsHeapSizeLimit} T:${adv.software.performanceMemory.totalJSHeapSize} U:${adv.software.performanceMemory.usedJSHeapSize}` : ''}><span className="text-gray-400">MEM:</span> {adv.software?.performanceMemory ? `${adv.software.performanceMemory.usedJSHeapSize} / ${adv.software.performanceMemory.totalJSHeapSize}` : 'N/A'}</div>
                              <div className="truncate" title={adv.software?.permissions ? `Geo: ${adv.software.permissions.geolocation}, Notif: ${adv.software.permissions.notifications}, Cam: ${adv.software.permissions.camera}, Mic: ${adv.software.permissions.microphone}, ClipR: ${adv.software.permissions['clipboard-read']}, ClipW: ${adv.software.permissions['clipboard-write']}` : ''}><span className="text-gray-400">PERMESSI:</span> {adv.software?.permissions ? `Geo: ${adv.software.permissions.geolocation?.slice(0,3)}, Notif: ${adv.software.permissions.notifications?.slice(0,3)}, Cam: ${adv.software.permissions.camera?.slice(0,3)}` : 'N/A'}</div>
                              ...
                              <div><span className="text-gray-400">MATH_ID:</span> <span className="font-bold text-gray-800" title={adv.software?.mathFingerprint ? JSON.stringify(adv.software.mathFingerprint) : ''}>{adv.software?.mathFingerprint?.pi ? "Presente" : "N/A"}</span></div>
